const About = () => {
    ...
    return (
        <section id="about" className="py-24 mx-auto max-w-3xl">
            <h2 className="flex items-center mb-8 text-2xl font-semibold text-copy-primary whitespace-nowrap after:content-[''] after:block after:relative after:top-px after:w-full after:h-px after:ml-4 after:bg-border">
                <span className="text-cta font-mono text-xl mr-2">01.</span>
                About Me
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-10">
                <div className="md:col-span-3 text-copy-secondary space-y-4">
                    <p>
                    ...
                    <ul className="grid grid-cols-2 gap-x-4 gap-y-2 font-mono text-sm list-none p-0">
                        {skills.map((skill, index) => (
                            <li key={index} className="relative pl-5 before:content-['▹'] before:absolute before:left-0 before:text-cta">
                                {skill}
                            </li>
                        ))}
                    </ul>
                ...
                <div className="md:col-span-2 relative group">
                    <div className="relative w-full max-w-xs mx-auto md:mx-0 aspect-square rounded">
                        {/* Placeholder Image */}
                        <div className="absolute inset-0 bg-card rounded-sm z-0 border-2 border-cta transition-transform duration-300 group-hover:translate-x-2 group-hover:translate-y-2"></div>
                        <img
                            className="absolute inset-0 w-full h-full object-cover rounded-sm z-10 grayscale transition-all duration-300 group-hover:grayscale-0"
